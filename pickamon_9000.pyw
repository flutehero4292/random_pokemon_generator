...
def gen_9():
    txt_output.delete("1.0", tk.END)
    txt_output.insert("1.0", pickamon(gen_9_mons))
def all_gens():
    txt_output.delete("1.0", tk.END)
...
btn_gen_9 = tk.Button(master=frm_buttons, text="Gen 9", command=gen_9)
btn_gen_9.pack(side=tk.LEFT, padx=3)
btn_all_gens = tk.Button(master=frm_buttons, text="All Gens", command=all_gens)
btn_all_gens.pack(side=tk.RIGHT, padx=6)

def bulbapedia(): #Adjusted to work with Type: Null due to the ": " in its name
    mon = txt_output.get("1.0", tk.END)
    name = list(mon.split(": "))[-1].title()
    if txt_output.get("1.0", "1.3") == "772":
        open_new_tab(f"https://bulbapedia.bulbagarden.net/wiki/Type:_Null_(Pok%C3%A9mon)")
    else:
